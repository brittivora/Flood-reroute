...
def _apply_overpass_mirror():
    """
    overpass-api.de (OSMnx's default Overpass endpoint) blocks requests
    from many cloud/data-center IP ranges (Render, AWS, etc.) to prevent
    automated abuse — this shows up as ConnectionRefusedError even though
    the same code works fine from a home network.

    Set OVERPASS_URL as an env var to point at a different public mirror
    if the default gets blocked from your deploy host. Left unset, OSMnx
    uses its normal default (fine for local development).
    """
    mirror = os.environ.get("OVERPASS_URL")
    if mirror:
        import osmnx as ox
        ox.settings.overpass_url = mirror
        print(f"[graph_utils] Using Overpass mirror: {mirror}")
...
def load_city_graph(place_name: str = "Greater Mumbai, Maharashtra, India",
    ...
    if not force_refresh and not os.path.exists(cache_path):
        cache_url = os.environ.get("GRAPH_CACHE_URL")
        if cache_url:
            print(f"[graph_utils] No local cache found — downloading from {cache_url} ...")
            resp = requests.get(cache_url, stream=True, timeout=300)
            resp.raise_for_status()
            with open(cache_path, "wb") as f:
                for chunk in resp.iter_content(chunk_size=1 << 20):
                    f.write(chunk)
            print(f"[graph_utils] Downloaded graph cache to {cache_path} "
